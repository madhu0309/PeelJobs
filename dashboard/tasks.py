...
from peeldb.models import (AppliedJobs, City, Company, User,
                           Facebook, FacebookGroup, FacebookPage, FacebookPost,
                           Industry, JobAlert, JobPost, Linkedin, LinkedinPost,
                           Qualification, SearchResult, SentMail, Skill,
                           Subscriber, TechnicalSkill, Ticket, Twitter,
                           TwitterPost, State)
...
def job_alerts_to_users():
    ...
    users = User.objects.filter(
        email_notifications=True, user_type='JS', is_bounce=False, is_unsubscribe=False,
        skills__skill__id__in=job_posts.values_list('skills', flat=True),
    ).distinct()
    for user in users:
        user_skills = user.skills.values_list('skill', flat=True)
        user_posts = job_posts.filter(skills__id__in=user_skills, location__in=[user.current_city])
        if user_posts:
            if user_posts.count() < 10:
                job_order = Case(*[When(pk=pk.id, then=pos) for pos, pk in enumerate(user_posts)])
                user_posts = user_posts | JobPost.objects.filter(skills__in=user_skills, status='Live')
...
def job_alerts_to_subscribers():
    ...
    jobs = JobPost.objects.filter(published_on__range=(from_date, to_date), status='Live')
    job_skills = jobs.values_list('skills', flat=True).distinct()
    for sub in Subscriber.objects.filter(skill__id__in=job_skills, is_verified=True).distinct('email'):
        skills = Subscriber.objects.filter(email=sub.email).values_list('skill__name', flat=True)
        sub_jobs = jobs.filter(skills__name__in=skills)
        if sub_jobs:
            if sub_jobs.count() < 10:
                job_order = Case(*[When(pk=pk.id, then=pos) for pos, pk in enumerate(sub_jobs)])
...
def job_alerts_to_alerts():
    from_date = (datetime.now() - timedelta(days=1))
    to_date = datetime.now()
    job_posts = JobPost.objects.filter(published_on__range=(from_date, to_date), status='Live')
    ...
    for alert in alerts:
        alert_jobs = job_posts.filter(Q(skills__in=alert.skill.all()) & Q(Q(location__in=alert.location.all()) | Q(industry__in=alert.industry.all()) |
                                      Q(max_year=alert.max_year) | Q(max_salary=alert.max_salary) | Q(min_salary=alert.min_salary)))
        if alert_jobs:
            job_order = Case(*[When(pk=pk.id, then=pos) for pos, pk in enumerate(alert_jobs)])
            if alert_jobs.count() < 10:
                alert_jobs = alert_jobs | JobPost.objects.filter(skills__in=alert.skill.all(), status='Live')
    ...
    for job in jobposts:
        ...
        if job.post_on_fb:
            ...
            posts = FacebookPost.objects.filter(
                job_post=job, page_or_group='group', is_active=True, post_status='Deleted')
        ...
        for user in users:
            if not AppliedJobs.objects.filter(user=user, job_post=job):
                AppliedJobs.objects.create(user=user, job_post=job, status='Pending', ip_address='', user_agent='')
        ...
        if job_post and access_token:
            ...
            if 'id' in response.keys():
                FacebookPost.objects.create(
                    job_post=job_post, page_or_group='page', page_or_group_id=facebook_id, post_id=response['id'], post_status='Posted')
...
def postonpeel_fb(job_post):
    if job_post:
        ...
        elif job_post.skills.all()[0].icon and job_post.skills.all()[0].status == 'Active':
    ...
    if user.is_fb_connected:
        ...
        if pages:
            ...
            for page in pages:
                if page.allow_post:
                    u = requests.post(
                        "https://graph.facebook.com/" + str(page.page_id) + "/feed", params, params=params)
                    response = u.json()
                    ...
                    if 'id' in response.keys():
                        FacebookPost.objects.create(
                            job_post=job_post, page_or_group=page, page_or_group_id=page.page_id, post_id=response['id'], post_status='Posted')
...
def poston_allfb_groups(job_post):
    ...
    with open('mpcomp/fb_groups.json') as data_file:
        ...
        for each in data:
            ...
            job_name = str(job_post.title) + ', for Exp ' + \
...
def postonlinkedin(user, job_post):
    ...
    if job_post:
        ...
        url = 'https://api.linkedin.com/v1/companies/' + settings.LN_COMPANYID + '/shares?format=json'
        ...
        kw = dict(
            data=json.dumps(post), params=params, headers=headers, timeout=60)
        response = requests.request('POST', url, **kw)
        response = response.json()
        if 'updatekey' in response.keys():
            LinkedinPost.objects.create(job_post=job_post, profile_or_group='Profile', post_id=response[
                                        'updatekey'], post_status='True', update_url=response['update_url'])
        if link:
        ...
        if "id" in response.keys():
            if page_or_profile == 'Profile':
                TwitterPost.objects.create(
                    job_post=job_post, page_or_profile=page_or_profile, post_id=response['id'], post_status='Posted')
            else:
                TwitterPost.objects.create(
                    job_post=job_post, page_or_profile=page_or_profile, post_id=response['id'], post_status='Posted')
...
def get_conditions(user):
    ...
    if user.expected_salary:
        if isinstance(user.expected_salary, float):
            conditions.append(
                Q(min_salary=float(user.expected_salary)))
            conditions.append(
                Q(max_salary=float(user.expected_salary)))
    if user.employment_history.all():
        conditions.append(Q(job_role__icontains=user.job_role))
    if user.education.all():
        conditions.append(
            Q(edu_qualification__in=user.education.all().values_list('degree', flat=True)))
    if user.skills.all():
        conditions.append(
            Q(skills__in=user.skills.all().values_list('id', flat=True)))
    return conditions
...
def applicants_notifications():
    ...
    current_date = datetime.strptime(
        str(datetime.now().date() - timedelta(days=10)), "%Y-%m-%d").strftime("%Y-%m-%d")
    today_applicants = User.objects.filter(email_notifications=True, profile_updated__lte=current_date, profile_completeness__lte=50,
                                           user_type='JS', is_bounce=False, is_unsubscribe=False)
    for user in today_applicants:
        conditions = get_conditions(user)
        if conditions:
            jobposts = JobPost.objects.filter(reduce(OR, conditions)).filter(status='Live').distinct()[:10]
        else:
...
def recruiter_jobpost_applicants():
    recruiters = User.objects.filter(user_type='RR', is_bounce=False, is_unsubscribe=False, email_notifications=True)
    current_date = datetime.strptime(
        str(datetime.now().date()), "%Y-%m-%d").strftime("%Y-%m-%d")
    for each in recruiters:
        if each.get_jobposts_count() > 1:
            job_posts = JobPost.objects.filter(user=each, status='Live', send_email_notifications=True)
            for job in job_posts:
                applicants = AppliedJobs.objects.filter(job_post=job, applied_on__date=current_date)
...
def send_dailyapplicantnotifications():
    ...
    current_date = dt.datetime.strptime(
        str(dt.datetime.now().date()), "%Y-%m-%d").strftime("%Y-%m-%d")
    daily_emails_count = db.users.find(
        {'date': current_date, 'mail_sent': True}).count()
    users = list(db.users.find({'$and': [{'email': {'$ne': ''}},
                                         {'location': {'$ne': ''}},
            ...
            try:
                ...
                for user in users:
                    ...
                    today = arrow.utcnow().to(
                        'Asia/Calcutta').format('DD.MM.YYYY HH:mm:ss')
                    pattern = '%d.%m.%Y %H:%M:%S'
...
@task()
def daily_report():
    current_date = datetime.strptime(str(datetime.now().date() - timedelta(days=1)), "%Y-%m-%d").strftime("%Y-%m-%d")
    ...
    today_total_recruiters = User.objects.filter(
        date_joined__contains=current_date).exclude(user_type='JS')
    ...
    today_total_recruiters = today_recruiters_count.count(
    ) + today_agency_recruiters_count.count()
    ...
    job_applied_users = User.objects.filter(id__in=today_job_applications.values('user'))
    today_tickets = Ticket.objects.filter(created_on__contains=current_date)

    formatted_date = datetime.strptime(
        str(current_date), "%Y-%m-%d").strftime("%d-%m-%Y")
    ...
    data = {
        ...
        'today_jobs_published_count': today_jobs_count.filter(status='Published').count(),
        ...
        'today_all_applicants_count': User.objects.filter(date_joined__contains=current_date, user_type='JS').count(),

        'today_full_time_jobs_count': today_full_time_jobs_count.count(),
        'today_full_time_draft_jobs_count': today_full_time_jobs_count.filter(status='Draft').count(),
        'today_full_time_pending_jobs_count': today_full_time_jobs_count.filter(status='Pending').count(),
        'today_full_time_published_jobs_count': today_full_time_jobs_count.filter(status='Published').count(),
        'today_full_time_live_jobs_count': today_full_time_jobs_count.filter(status='Live').count(),
        'today_full_time_disabled_jobs_count': today_full_time_jobs_count.filter(status='Disabled').count(),

        'today_govt_jobs_count': today_govt_jobs_count.count(),
        'today_govt_jobs_draft_count': today_govt_jobs_count.filter(status='Draft').count(),
        'today_govt_jobs_pending_count': today_govt_jobs_count.filter(status='Pending').count(),
        'today_govt_jobs_published_count': today_govt_jobs_count.filter(status='Published').count(),
        'today_govt_jobs_live_count': today_govt_jobs_count.filter(status='Live').count(),
        'today_govt_jobs_disabled_count': today_govt_jobs_count.filter(status='Disabled').count(),

        'today_internship_jobs_count': today_internship_jobs_count.count(),
        'today_internship_jobs_draft_count': today_internship_jobs_count.filter(status='Draft').count(),
        'today_internship_jobs_pending_count': today_internship_jobs_count.filter(status='Pending').count(),
        'today_internship_jobs_published_count': today_internship_jobs_count.filter(status='Published').count(),
        'today_internship_jobs_live_count': today_internship_jobs_count.filter(status='Live').count(),
        'today_internship_jobs_disabled_count': today_internship_jobs_count.filter(status='Disabled').count(),

        'today_walkin_jobs_count': today_walkin_jobs_count.count(),
        'today_walkin_jobs_draft_count': today_walkin_jobs_count.filter(status='Draft').count(),
        'today_walkin_jobs_pending_count': today_walkin_jobs_count.filter(status='Pending').count(),
        'today_walkin_jobs_published_count': today_walkin_jobs_count.filter(status='Published').count(),
        'today_walkin_jobs_live_count': today_walkin_jobs_count.filter(status='Live').count(),
        'today_walkin_jobs_disabled_count': today_walkin_jobs_count.filter(status='Disabled').count(),

        'today_admin_jobs_count': today_admin_jobs_count.count(),
        'today_admin_draft_jobs_count': today_admin_jobs_count.filter(status='Draft').count(),
        'today_admin_pending_jobs_count': today_admin_jobs_count.filter(status='Pending').count(),
        'today_admin_published_jobs_count': today_admin_jobs_count.filter(status='Published').count(),
        'today_admin_live_jobs_count': today_admin_jobs_count.filter(status='Live').count(),
        'today_admin_disabled_jobs_count': today_admin_jobs_count.filter(status='Disabled').count(),

        'today_admin_full_time_draft_jobs_count': today_admin_full_time_jobs_count.filter(status='Draft').count(),
        'today_admin_full_time_pending_jobs_count': today_admin_full_time_jobs_count.filter(status='Pending').count(),
        'today_admin_full_time_published_jobs_count': today_admin_full_time_jobs_count.filter(status='Published').count(),
        'today_admin_full_time_jobs_count': today_admin_full_time_jobs_count.count(),
        'today_admin_full_time_live_jobs_count': today_admin_full_time_jobs_count.filter(status='Live').count(),
        'today_admin_full_time_disabled_jobs_count': today_admin_full_time_jobs_count.filter(status='Disabled').count(),

        'today_admin_govt_jobs_count': today_admin_govt_jobs_count.count(),
        'today_admin_govt_draft_jobs_count': today_admin_govt_jobs_count.filter(status='Draft').count(),
        'today_admin_govt_pending_jobs_count': today_admin_govt_jobs_count.filter(status='Pending').count(),
        'today_admin_govt_published_jobs_count': today_admin_govt_jobs_count.filter(status='Published').count(),
        'today_admin_govt_live_jobs_count': today_admin_govt_jobs_count.filter(status='Live').count(),
        'today_admin_govt_disabled_jobs_count': today_admin_govt_jobs_count.filter(status='Disabled').count(),

        'today_admin_internship_jobs_count': today_admin_internship_jobs_count.count(),
        'today_admin_internship_draft_jobs_count': today_admin_internship_jobs_count.filter(status='Draft').count(),
        'today_admin_internship_pending_jobs_count': today_admin_internship_jobs_count.filter(status='Pending').count(),
        'today_admin_internship_published_jobs_count': today_admin_internship_jobs_count.filter(status='Published').count(),
        'today_admin_internship_live_jobs_count': today_admin_internship_jobs_count.filter(status='Live').count(),
        'today_admin_internship_disabled_jobs_count': today_admin_internship_jobs_count.filter(status='Disabled').count(),

        'today_admin_walkin_jobs_count': today_admin_walkin_jobs_count.count(),
        'today_admin_walkin_draft_jobs_count': today_admin_walkin_jobs_count.filter(status='Draft').count(),
        'today_admin_walkin_pending_jobs_count': today_admin_walkin_jobs_count.filter(status='Pending').count(),
        'today_admin_walkin_published_jobs_count': today_admin_walkin_jobs_count.filter(status='Published').count(),
        'today_admin_walkin_live_jobs_count': today_admin_walkin_jobs_count.filter(status='Live').count(),
        'today_admin_walkin_disabled_jobs_count': today_admin_walkin_jobs_count.filter(status='Disabled').count(),

        'today_applicants_count': today_applicants_count.count(),

        'today_login_only_once_applicants_count': today_applicants_count.filter(is_login=False).count(),
        'today_resume_applicants_count': today_applicants_count.exclude(resume='').count(),
        'today_profile_applicants_count': today_applicants_count.filter(profile_completeness__gte=50).count(),

        'today_applied_applicants_count': job_applied_users.filter(registered_from='Social').count(),
        'resume_applicants_count': resume_applicants_count.count(),
        'resume_login_once_applicants_count': resume_applicants_count.filter(is_login=False).count(),
        'resume_uploaded_applicants_count': resume_applicants_count.exclude(resume='').count(),
        'resume_profile_applicants_count': resume_applicants_count.filter(profile_completeness__gte=50).count(),
        'resume_applied_applicants_count': job_applied_users.filter(registered_from='Resume').count(),

        'resumepool_applicants': resumepool_applicants.count(),
        'resumepool_login_once_applicants': resumepool_applicants.filter(is_login=False).count(),
        'resumepool_profile_applicants': resumepool_applicants.filter(profile_completeness__gte=50).count(),
        'resumepool_applied_applicants': job_applied_users.filter(registered_from='Resume').count(),

        'today_register_applicants_count': today_register_applicants_count.count(),

        'today_register_login_only_once_applicants_count': today_register_applicants_count.filter(is_login=False).count(),
        'today_register_resume_applicants_count': today_register_applicants_count.filter().exclude(resume='').count(),
        'today_register_profile_applicants_count': today_register_applicants_count.filter(profile_completeness__gte=50).count(),

        'today_register_applied_applicants_count': job_applied_users.filter(registered_from='Email').count(),
...
def applicants_profile_update_notifications_two_hours():
    today_applicants = User.objects.filter(user_type='JS', profile_completeness__lt=50, is_unsubscribe=False,
                                           is_bounce=False, email_notifications=True).exclude(email__icontains='micropyramid.com')
    for user in today_applicants:
...
def applicants_profile_update_notifications():
    ...
    for each in today_applicants:
        ...
        if days == 4 or days == 6 or days % 7 == 0:
            skills = Skill.objects.filter(id__in=each.skills.all().values_list('skill_id', flat=True))
            if skills:
                job_posts = JobPost.objects.filter(status='Live', skills__in=skills)[:15]
            else:
    ...
    recruiters = User.objects.filter(Q(Q(user_type='RR') | Q(user_type='AA')) & Q(
        email_notifications=True, is_unsubscribe=False, is_bounce=False, is_active=False))
    ...
    inactive_users = User.objects.filter(is_unsubscribe=False, email_notifications=True,
                                         is_bounce=False, is_active=False, user_type='JS')
    ...
    users = User.objects.filter(user_type='JS', resume='', email_notifications=True, date_joined__contains=day.date(),
...
def applicants_walkin_job_notifications():
    ...
    today_applicants = User.objects.filter(user_type='JS', is_unsubscribe=False, is_bounce=False,
...
def applicants_job_notifications():
    users = User.objects.filter(user_type='JS', is_unsubscribe=False, is_bounce=False,
                                email_notifications=True).exclude(email__icontains='micropyramid.com')
    for user in users:
        job_posts = []
        if user.skills.all():
            tech_skills = Skill.objects.filter(
                id__in=user.skills.all().values('skill'))
            job_posts = JobPost.objects.filter(
                status='Live', skills__in=tech_skills)
        ...
        user_active = True if user.is_active else False
        Memail(mto, mfrom, subject, rendered, user_active)
    users = User.objects.filter(user_type='JS', email_notifications=True, is_unsubscribe=False, is_bounce=False)
...
def alerting_applicants():
    ...
    users = User.objects.filter(user_type='JS', email_notifications=True, is_unsubscribe=False,
    ...
    current_date = datetime.strptime(str(datetime.now().date()), "%Y-%m-%d").strftime("%m-%d")
...
def sending_mobile_campaign():
    users = list(db.users.find({'$and': [{'mobile': {'$ne': ''}}, {
                 'location': 'Hyderabad'}, {'sms_campaign_sent': {'$exists': False}}]}))
    count = 0
    current_date = datetime.strptime(
        str(datetime.now().date()), "%Y-%m-%d").strftime("%Y-%m-%d")
    ...
    for user in users:
        ...
        requests.get('http://sms.9sm.in/rest/services/sendSMS/sendGroupSms?AUTH_KEY=' + str(SMS_AUTH_KEY) + '&message=' + str(
            message) + '&senderId=' + str(BULK_SMS_FROM) + '&routeId=3&mobileNos=' + str(mobile) + '&smsContentType=english')
        db.users.update({'mobile': mobile}, {'$set': {'sms_campaign_sent': True,
                                                      'sms_date': current_date}}, False, True)
        count = count + 1
...
def sitemap_generation():
    ...
    if no_job_locations_xml_cont != xml_cont:
        ...
        no_job_locations_xml_file = open('../sitemap/sitemap-locations-without-jobs.xml', 'w')
    ...
    all_jobs = JobPost.objects.filter(
        job_type='internship', status='Live').values_list('location', flat=True).distinct()
    ...
    no_job_skills_walkin_xml_file = open('../sitemap/sitemap-skill-without-walkins.xml', 'w')
    ...
    for index, each in enumerate(locations):
        ...
        for location in each:
            for skill in skills:
                jobs = JobPost.objects.filter(status='Live', skills=skill, location=location).count()
        ...
        skills_location_xml_file = open('../sitemap/sitemap-skill-locations-' + str(index) + '.xml', 'w')
        skills_location_xml_file.write(skills_locations_xml_cont)
        no_job_skills_location_xml_file = open('../sitemap/sitemap-skill-locations-without-jobs-' + str(index) + '.xml', 'w')
        no_job_skills_location_xml_file.write(no_job_skills_locations_xml_cont)

        skills_locations_walkins_xml_cont = skills_locations_walkins_xml_cont + '</urlset>'
        no_job_skills_locations_walkins_xml_cont = no_job_skills_locations_walkins_xml_cont + '</urlset>'

        skills_location_walkins_xml_file = open('../sitemap/sitemap-skill-location-walkins-' + str(index) + '.xml', 'w')
        skills_location_walkins_xml_file.write(skills_locations_walkins_xml_cont)
        no_job_skills_location_walkins_xml_file = open('../sitemap/sitemap-skill-location-without-walkins-' + str(index) + '.xml', 'w')
        no_job_skills_location_walkins_xml_file.write(no_job_skills_locations_walkins_xml_cont)
    ...
    for index, each in enumerate(locations):
        ...
        skills_location_fresher_xml_file = open('../sitemap/sitemap-skill-location-fresher-jobs-' + str(index) + '.xml', 'w')
        skills_location_fresher_xml_file.write(skills_location_fresher_xml_cont)

        no_job_skills_location_fresher_xml_file = open('../sitemap/sitemap-skill-location-without-fresher-jobs-' + str(index) + '.xml', 'w')
        no_job_skills_location_fresher_xml_file.write(no_job_skills_location_fresher_xml_cont)
    ...
    no_job_locations_walkin_xml_file = open('../sitemap/sitemap-location-without-walkins.xml', 'w')
    ...
    locations_fresher_jobs_xml_file = open('../sitemap/sitemap-location-fresher-jobs.xml', 'w')
    locations_fresher_jobs_xml_file.write(locations_fresher_jobs_xml_cont)
    no_job_locations_fresher_jobs_xml_cont = no_job_locations_fresher_jobs_xml_cont + '</urlset>'
    no_job_locations_fresher_jobs_xml_file = open('../sitemap/sitemap-location-without-fresher-jobs.xml', 'w')
    ...
    no_job_skills_fresher_xml_file = open('../sitemap/sitemap-skill-without-fresher-jobs.xml', 'w')
    ...
    recruiter_xml_file.write(recruiters_xml_cont.encode('ascii', 'ignore').decode('ascii'))
    ...
    government_jobs = JobPost.objects.filter(
        status='Live', job_type='government')
    jobposts = list(
...
def handle_sendgrid_bounces():
    bounces = requests.get('https://api.sendgrid.com/api/bounces.get.json?api_user='+settings.SG_USER+'&api_key='+settings.SG_PWD)
...
def sending_mails_to_applicants_sendgrid():
    users = list(db.users.find({'$and': [{'email': {'$ne': ''}}, {'is_bounce': {'$exists': False}}, {'unsubscribe': {'$exists': False}}]}))
    i = 0
    for user in users:
        if i <= 10000:
...
def check_meta_data():
    ...
    urls = [{'url': host_name + '/', 'name': 'home_page'},
            ...
            {'url': host_name + '/java-walkins-in-bangalore/', 'name': 'skill_location_walkin_jobs'},
            {'url': host_name + '/java-fresher-jobs/', 'name': 'skill_fresher_jobs'},
            {'url': host_name + '/bangalore-fresher-jobs/', 'name': 'location_fresher_jobs'},
            {'url': host_name + '/java-fresher-jobs-in-bangalore/', 'name': 'skill_location_fresher_jobs'},
            {'url': host_name + '/bpo-industry-jobs/', 'name': 'industry_jobs'},
            {'url': host_name + '/internship-in-bangalore/', 'name': 'location_internship_jobs'},
            ...
            {'url': host_name + '/jobs-by-location/', 'name': 'jobs_by_location', 'job_type': 'jobs'},
            ...
            {'url': host_name + '/blog/how-to-develop-a-successful-career-plan/', 'name': 'blog_view'},
            ...
            {'url': host_name + '/jobposts/year/2017/month/1/date/1/', 'name': 'day_calendar'},
            {'url': host_name + '/jobs-by-degree/', 'name': 'jobs_by_degree'},
            {'url': host_name + '/junior-python-developer-0-to-2-years-1046/', 'name': 'job_detail_page'},
            {'url': host_name + '/post-job/', 'name': 'post_job'},
            {'url': host_name + '/recruiter/login/', 'name': 'recruiter_login'},
            {'url': host_name + '/walkin-jobs-by-skills/', 'name': 'fresher_jobs_by_skills', 'job_type': 'walkin'},
            {'url': host_name + '/fresher-jobs-by-skills/', 'name': 'fresher_jobs_by_skills', 'job_type': 'fresher'},
    ...
    test_data = {'skill': 'Java', 'city': 'Bangalore', 'current_page': '1', 'industry': 'BPO / Call Centre / ITES', 'company': {'name': 'Integraph'},
                 'tag': {'name': 'SEO'}, 'user': {'username': 'smitra'}, 'blog_name': {'title': 'How to develop a successful career plan?'},
                 'category': {'name': 'Job Search'}, 'degree': 'B.Com', 'month': 'January 2017', 'searched_month': 'January', 'date': 1, 'year': 2017,
                 'job': {'title': 'Junior Python Developer', 'job_role': 'Software Developer', 'min_year': 0, 'max_year': 2,
                         'location': {'all': [{'name': 'Hyderabad'}]}}, 'search': 'Java, B.Com'}
    for each in urls:
        ...
        url_meta_description = [meta.attrs['content'] for meta in metas if 'name' in meta.attrs and meta.attrs['name'] == 'description']
        url_meta_title = [meta.attrs['content'] for meta in metas if 'property' in meta.attrs and meta.attrs['property'] == 'og:title']
        metas = soup.find_all('h1')
        url_h1_tag = [meta.text for meta in metas if 'class' in meta.attrs and meta.attrs['class'] == 'internship-text']
        if url_h1_tag:
        ...
        each.update({'is_h1_tag_correct': is_h1_tag_correct, 'is_meta_description_correct': is_meta_description_correct,
                     'is_meta_title_correct': is_meta_title_correct, 'url_meta_title[0]': url_meta_title,
                     'url_meta_description[0]': url_meta_description, 'url_h1_tag[0]': url_h1_tag,
                     'meta_title': meta_title, 'meta_description': meta_description, 'h1_tag': h1_tag})
        db.meta_data.update({'name': each['name']}, {'$set': {'is_h1_tag_correct': is_h1_tag_correct,
