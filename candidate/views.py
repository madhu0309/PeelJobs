...
from mpcomp.views import (jobseeker_login_required, get_prev_after_pages_count, mongoconnection, get_social_referer,
                          Memail, get_resume_data, handle_uploaded_file, get_meta)
from candidate.forms import PersonalInfoForm, ProfileDescriptionForm, WorkExperienceForm, EducationForm,\
    DegreeForm, EducationInstitueForm, TechnicalSkillForm, ProjectForm, \
    ProfessinalInfoForm, YEARS, MONTHS, DEGREE_TYPES, JobAlertForm
from peeldb.models import (User, City, Country, UserEmail, Industry, Language, UserLanguage, Subscriber,
                           EmploymentHistory, Project, MARTIAL_STATUS, EducationDetails, Degree, EducationInstitue,
                           Skill, TechnicalSkill, Qualification, JobPost, TechnicalSkill_STATUS, JobAlert, FunctionalArea,
                           Question, AssessmentData, Solution, State)
...
def index(request):
    ...
    data = {'jobs_list': latest_jobs_list,
...
def applicant_email_unsubscribing(request, email_type, message_id):
    ...
    if request.POST:
        if request.POST.get('reason') and user:
            ...
            return HttpResponse(json.dumps({'error': True, 'err_message': 'You are alredy Unsubscribed'}))
        else:
            return HttpResponse(json.dumps({'error': True, 'message': '* Please provide a reason!'}))
    if not user:
...
def bounces(request):
    ...
    if js["Type"] == "Notification":
        ...
        if 'notificationType' in arg_info.keys() and arg_info['notificationType'] == 'Bounce':
        ...
        if 'content' in arg_info.keys():
            ...
            bounced_email = html_content.replace(
                '\n', '').replace('rfc822;', '')
            db.users.update(
                {'email': bounced_email}, {'$set': {'is_bounce': True}})
...
def upload_resume(request):
    ...
    if 'resume' in request.FILES and request.user.user_type == 'JS':
        ...
        data = {'error': True, 'data': 'Upload your resume either in Doc or Docx or PDF or ODT format'}
    return HttpResponse(json.dumps(data))
...
def upload_profilepic(request):
    ...
    if 'profile_pic' in request.FILES:
        ...
        if str(ftype) in sup_formates:
            ...
            data = {'error': True, 'data': 'Please upload valid formats For Ex: JPEG, JPG, PNG format'}
    else:
        data = {'error': True, 'data': 'Upload your profile picture either in JPEG, JPG, PNG format'}
    return HttpResponse(json.dumps(data))
...
def edit_personalinfo(request):
    if request.method == "POST":
        validate_personalform = PersonalInfoForm(
        ...
        if validate_personalform.is_valid():
            ...
            if request.POST.get('dob'):
                ...
                user.dob = dob
            user.marital_status = request.POST.get(
                'marital_status') if request.POST.get('marital_status') else None
            user.first_name = request.POST.get('first_name')
            user.last_name = request.POST.get(
                'last_name') if request.POST.get('last_name') else None
            user.alternate_mobile = request.POST.get(
                'alternate_mobile') if request.POST.get('alternate_mobile') else None
            user.pincode = request.POST.get('pincode') if request.POST.get('pincode') else None
            user.gender = request.POST.get('gender') if request.POST.get('gender') else None
            user.address = request.POST.get('address')
            user.permanent_address = request.POST.get('permanent_address')
            user.resume_title = request.POST.get(
                'resume_title') if request.POST.get('resume_title') else None
            user.nationality = 1
            ...
            user.last_mobile_code_verified_on = datetime.datetime.now(
                timezone.utc)
            ...
            data = {
                'error': False, 'response': 'Presonal Info edited successfully',
                'profile_percantage': request.user.profile_completion_percentage,
                'personal_info': True,
                'first_name': user.first_name,
                'mobile': user.mobile,
                'resume_title': user.resume_title,
                'dob': datetime.datetime.strptime(str(user.dob), "%Y-%m-%d").strftime('%b %d, %Y'),
        ...
        cities = City.objects.filter(state__country_id=1, status='Enabled').order_by('name')
        countries = Country.objects.all()
        martial_status = MARTIAL_STATUS
        industires = Industry.objects.filter(status="Active").order_by('name').exclude(id=36)
        functional_areas = FunctionalArea.objects.filter(status="Active").order_by('name')
        data = {'martial_status': martial_status,
                ...
                'functional_areas': functional_areas}
        template = 'mobile/profile/edit_personalinfo.html' if request.is_mobile else 'candidate/edit_personalinfo.html'
        return render(request, template, data)
...
def edit_profile_description(request):
    if request.method == "POST":
        validate_personalform = ProfileDescriptionForm(request.POST, instance=request.user)
        if validate_personalform.is_valid():
            ...
            data = {'error': True, 'response': validate_personalform.errors}
        return HttpResponse(json.dumps(data))
    template = 'mobile/profile/edit_profile_description.html' if request.is_mobile else 'candidate/edit_profile_description.html'
...
def edit_professionalinfo(request):
    if request.method == "POST":
        validate_professionalinfo = ProfessinalInfoForm(request.POST, instance=request.user)
        if validate_professionalinfo.is_valid():
            ...
            user.save()
            data = {
                'error': False, 'info': 'professional info edited successfully',
                'profile_percantage': request.user.profile_completion_percentage,
        ...
        data = {'industires': Industry.objects.filter(status='Active').exclude(id=36),
                'years': YEARS, 'months': MONTHS}
        template = 'mobile/profile/edit_professionalInfo.html' if request.is_mobile else 'candidate/edit_professionalInfo.html'
        return render(request, template, data)
...
def add_language(request):
    if request.method == 'GET':
        languages = Language.objects.all()
        template = 'mobile/profile/add_language.html' if request.is_mobile else 'candidate/add_language.html'
    ...
    if request.POST.get('language'):
        if request.user.language.filter(language_id=request.POST.get('language')):
            data = {
                'error': True, 'response_message': 'You have already created this langugae in your profile'}
            return HttpResponse(json.dumps(data))
        ...
        if read or write or speak:
            ...
            data = {'error': True, 'language': 'Please Select atleast any read/write/speak  option'}
        return HttpResponse(json.dumps(data))
    else:
...
def edit_language(request, language_id):
    user_language = UserLanguage.objects.filter(
        user=request.user, id=language_id)
    if request.method == 'GET':
        languages = Language.objects.all()
        if user_language:
            template = 'mobile/profile/editlanguage.html' if request.is_mobile else 'candidate/editlanguage.html'
            data = {'language': user_language[0], 'languages': languages}
        else:
            template = 'mobile/404.html' if request.is_mobile else '404.html'
            data = {'message': 'Sorry, User with this language not exists',
    ...
    if request.POST.get('get_lang'):
        ...
        data = {'error': False, 'id': lan.language.id, 'read': lan.read, 'write': lan.write, 'speak': lan.speak}
        return HttpResponse(json.dumps(data))
    if request.POST.get('edit_lang') and request.POST.get('language'):
        if request.user.language.filter(language_id=request.POST.get('language')).exclude(id=language_id):
            data = {'error': True, 'response': 'You have already created this langugae in your profile'}
            return HttpResponse(json.dumps(data))
        read = request.POST.get('read') == "on"
        write = request.POST.get('write') == "on"
        speak = request.POST.get('speak') == "on"
        if read or write or speak:
            ...
            data = {'error': True, 'response': 'Please Select atleast any read/write/speak option'}
        return HttpResponse(json.dumps(data))
    else:
        data = {'error': True, 'response': 'Please select the language'}
        return HttpResponse(json.dumps(data))
...
def add_experience(request):
    if request.method == 'GET':
        template = 'mobile/profile/add_experience.html' if request.is_mobile else 'candidate/add_experience.html'
        return render(request, template)
    work_experience = WorkExperienceForm(request.POST)
    if work_experience.is_valid():
        if request.user.employment_history.filter(company=request.POST.get('company'), designation=request.POST.get('designation')):
            data = {'error': True, 'response_message':
                    'Experince with this company and designation already exists'}
            return HttpResponse(json.dumps(data))
        ...
        if request.POST.get('current_job') == "on":
            ...
            to = datetime.datetime.strptime(
                request.POST.get('to_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
            experience.to_date = to
        experience.from_date = datetime.datetime.strptime(
            request.POST.get('from_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
...
def edit_experience(request, experience_id):
    ...
    if request.method == 'GET':
        if experiences:
            ...
            template = 'mobile/profile/edit_experience.html' if request.is_mobile else 'candidate/edit_experience.html'
            data = {'experience': experience}
        else:
            template = 'mobile/404.html' if request.is_mobile else '404.html'
            data = {'message': 'Sorry, User with this Experience not exists',
    ...
    if request.user.employment_history.filter(company=request.POST.get('company'), designation=request.POST.get('designation')).exclude(id=experience_id):
        data = {'error': True, 'response_message':
                'Experince with this company and designation already exists'}
        return HttpResponse(json.dumps(data))
    else:
        if work_experience.is_valid():
            ...
            data = {
                'error': False, 'response': 'work history updated successfully'}
        else:
            data = {'error': True, 'response': work_experience.errors}
        return HttpResponse(json.dumps(data))
...
def add_education(request):
    if request.method == 'GET':
        template = 'mobile/profile/add_education.html' if request.is_mobile else 'candidate/add_education.html'
        return render(request, template, {'degree_types': DEGREE_TYPES,
    ...
    if education_valid.is_valid() and degree_valid.is_valid() and education_institute_valid.is_valid():
        if request.user.education.filter(degree__degree_name=request.POST.get('degree_name')):
            return HttpResponse(json.dumps({'error': True, 'response_message': 'education already exists'}))
        ...
        degree = Degree.objects.create(degree_name=degree_name, degree_type=request.POST.get(
            'degree_type'), specialization=request.POST.get('specialization'))
        to = None if (request.POST.get('current_education') == "on") else datetime.datetime.strptime(
            request.POST.get('to_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
        from_date = datetime.datetime.strptime(
            request.POST.get('from_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
        education = EducationDetails.objects.create(institute=institute, degree=degree, score=request.POST.get(
            'score'), from_date=from_date, to_date=to, current_education=request.POST.get('current_education') == "on")
        request.user.profile_updated = datetime.datetime.now(timezone.utc)
        request.user.save()
        request.user.education.add(education)
        return HttpResponse(json.dumps({'error': False, 'response': 'education details added'}))
    else:
        errors = {}
...
def edit_education(request, education_id):
    ...
    if education:
        if request.method == 'GET':
            template = 'mobile/profile/edit_education.html' if request.is_mobile else 'candidate/edit_education.html'
            return render(request, template, {'degree_types': DEGREE_TYPES,
                                              'qualifications': Qualification.objects.filter(status='Active').order_by('name'),
                                              'education': education,
                                              'cities': City.objects.filter(status='Enabled').order_by('name')})
        ...
        if education_valid.is_valid() and degree_valid.is_valid() and education_institute_valid.is_valid():
            ...
            if request.user.education.filter(degree__degree_name=request.POST.get('degree_name')).exclude(id=education_id):
                return HttpResponse(json.dumps({'error': True, 'response_message': 'Education already exists'}))
            ...
            education.from_date = datetime.datetime.strptime(
                request.POST.get('from_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
            education.to_date = None if (request.POST.get('current_education') == "on") else datetime.datetime.strptime(
                request.POST.get('to_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
            education.score = request.POST.get('score')
            education.current_education = request.POST.get(
                'current_education') == "on"
            education.save()
            user = request.user
            user.profile_updated = datetime.datetime.now(timezone.utc)
            user.save()

            return HttpResponse(json.dumps({'error': False, 'response': 'education updated successfully'}))
        else:
            errors = {}
            for k in education_valid.errors:
                errors[k] = education_valid.errors[k][0]
        ...
        return render(request, template, {'message': 'Sorry, User with this Education not exists',
...
def add_technicalskill(request):
    if request.method == 'GET':
        ...
        skills = Skill.objects.filter(
            status='Active').exclude(id__in=user_skills).order_by('name')
        template = 'mobile/profile/add_technicalskill.html' if request.is_mobile else 'candidate/add_technicalskill.html'
        return render(request, template, {'years': YEARS, 'months': MONTHS, 'skills': skills,
                                          'status': TechnicalSkill_STATUS})
    technical_skill = TechnicalSkillForm(
        request.POST, requested_user=request.user)
    if technical_skill.is_valid():
        if request.user.skills.filter(skill__id=request.POST.get('skill')):
            data = {
                'error': True, 'response_message': 'You have already created this skill in your profile'}
            return HttpResponse(json.dumps(data))
        skill = technical_skill.save()
        if request.POST.get('last_used'):
            last_used = datetime.datetime.strptime(
                request.POST.get('last_used'), "%m/%d/%Y").strftime("%Y-%m-%d")
        ...
        data = {'error': False, 'response': 'techskills added',
                'profile_percantage': request.user.profile_completion_percentage,
...
def edit_technicalskill(request, technical_skill_id):
    ...
    if skill:
        if request.method == 'GET':
            skills = Skill.objects.filter(status='Active').order_by('name')
            template = 'mobile/profile/edit_technicalskill.html' if request.is_mobile else 'candidate/edit_technicalskill.html'
            return render(request, template, {'years': YEARS,
                                              'months': MONTHS,
                                              'skills': skills,
        ...
        if technical_skill.is_valid():
            if request.user.skills.filter(skill_id=request.POST.get('skill')).exclude(id=technical_skill_id):
                data = {
                    'error': True, 'response_message': 'You have already created this skill in your profile'}
                return HttpResponse(json.dumps(data))
            ...
            data = {
                'error': False, 'response': 'skillinfo updated successfully'}
        else:
            data = {'error': True, 'response': technical_skill.errors}
        return HttpResponse(json.dumps(data))
    else:
...
def add_project(request):
    if request.method == 'GET':
        template = 'mobile/profile/add_project.html' if request.is_mobile else 'candidate/add_project.html'
        return render(request, template, {'skills': Skill.objects.filter(status='Active'),
                                          'cities': City.objects.filter(status='Enabled')})
    project_form = ProjectForm(request.POST)
    if project_form.is_valid():
        if User.objects.filter(id=request.user.id, project__name=request.POST.get('name')):
            data = {
                'error': True, 'response_message': 'Project already exists'}
            return HttpResponse(json.dumps(data))
...
def edit_project(request, project_id):
    ...
    if projects:
        if request.method == 'GET':
            template = 'mobile/profile/edit_project.html' if request.is_mobile else 'candidate/edit_project.html'
            return render(request, template, {'project': projects[0],
                                              'skills': Skill.objects.filter(status='Active'),
                                              'cities': City.objects.filter(status='Enabled')})
        project_form = ProjectForm(request.POST, instance=projects[0])
        if project_form.is_valid():
            if request.user.project.filter(name=request.POST.get('name')).exclude(id=project_id):
                data = {
                    'error': True, 'response_message': 'Project already exists'}
                return HttpResponse(json.dumps(data))
            project = project_form.save()
...
def edit_email(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            requested_email = UserEmail.objects.get(
        ...
        template = 'mobile/profile/edit_email.html' if request.is_mobile else 'candidate/edit_email.html'
        return render(request, template)
...
def job_alert(request):
    if request.method == 'GET':
        ...
        template = 'mobile/alert/job_alert.html' if request.is_mobile else 'alert/job_alert.html'
        return render(request, template, {'skills': Skill.objects.filter(status='Active'),
    ...
    if validate_jobalert.is_valid():
        ...
        job_alert.email = request.user.email if request.user.is_authenticated else request.POST.get('email', '')
        while True:
        ...
        url = request.scheme + '://' + request.META['HTTP_HOST'] + "/alert/verification/" + str(job_alert.subscribe_code) + "/"
        c = {'alert': job_alert, 'user': request.user, 'new_alert': True, 'verify_alert': True, 'redirect_url': url}
        rendered = temp.render(c)
        user_active = True if request.user.is_authenticated and request.user.is_active else False
        Memail(request.POST.get('email'), mfrom, subject, rendered, user_active)
        return HttpResponse(json.dumps({'error': False, 'message': 'job alert created successfully',
                                        'alert_id': job_alert.id}))
    else:
        return HttpResponse(json.dumps({'error': True, 'message': validate_jobalert.errors}))
...
def alert_subscribe_verification(request, obj_type, obj_id):
    ...
    return HttpResponseRedirect('/jobs/?verification=error&type=' + ('Alerts' if obj_type == 'alert' else 'Subscribers'))


def job_alert_results(request, job_alert_id):
    if request.user.is_authenticated and request.user.user_type == 'JS':
        job_alerts = JobAlert.objects.filter(
            id=job_alert_id, email=request.user.email)
    else:
    ...
    if job_alerts:
        ...
        jobs_list = JobPost.objects.filter((Q(skills__in=job_alert.skill.all(), status="Live")) &
                                           (Q(industry__in=job_alert.industry.all()) |
                                            ...
                                            Q(location__in=job_alert.location.all()))).distinct()
        if not jobs_list:
            jobs_list = JobPost.objects.filter(
                status='Live', skills__in=job_alert.skill.all())
        items_per_page = 10
        no_pages = int(
            math.ceil(float(jobs_list.count()) / items_per_page))
        ...
        data = {'jobs_list': jobs_list,
                ...
                'current_page': page,
                'last_page': no_pages}
        template = 'mobile/alert/job_alert_results.html' if request.is_mobile else 'alert/job_alert_results.html'
        return render(request, template, data)
    else:
...
def modify_job_alert(request, job_alert_id):
    ...
    if job_alerts:
        job_alert = job_alerts[0]
        if request.method == 'GET':
            data = {'skills': Skill.objects.filter(status='Active'),
                    'industires': Industry.objects.filter(status='Active'),
                    'cities': City.objects.filter(status='Enabled'),
                    'years': YEARS, 'job_alert': job_alert}
            template = 'mobile/alert/modify_job_alert.html' if request.is_mobile else 'alert/modify_job_alert.html'
            return render(request, template, data)
        ...
        if validate_jobalert.is_valid():
            ...
            data = {'error': False, 'message':
                    'job alert updated successfully', 'alert_id': job_alert.id}
        else:
...
def alerts_list(request, **kwargs):
    ...
    if request.user.is_authenticated:
        ...
        prev_page, previous_page, aft_page, after_page = get_prev_after_pages_count(
            page, no_pages)
        template = 'mobile/alert/list.html' if request.is_mobile else 'alert/list.html'
        return render(request, template, {'job_alerts': job_alerts,
                                          'aft_page': aft_page,
                                          'after_page': after_page,
                                          'prev_page': prev_page,
                                          'previous_page': previous_page,
...
def edit_emailnotifications(request):
    if request.method == 'POST':
        ...
        user.save()
        data = {'status': user.email_notifications, 'response': 'Updated Successfully', 'error': False}
    else:
        data = {'error': True}
    return HttpResponse(json.dumps(data))
...
def user_password_change(request):
    if request.method == 'POST':
        ...
        if validate_changepassword.is_valid():
            ...
            if request.POST['new_password'] != request.POST['retype_password']:
                return HttpResponse(json.dumps({'error': True, 'response_message': 'Password and ConfirmPasswords did not match'}))
    ...
    template = 'mobile/profile/change_user_password.html' if request.is_mobile else 'candidate/change_user_password.html'
    return render(request, template)
...
def question_view(request, que_id):
    ...
    if question:
        ...
        return render(request, 'assessments/question_view.html', {'question': question, 'meta_title': meta_title,
                                                                  'meta_description': meta_description, 'h1_tag': h1_tag,
                                                                  'latest_questions': latest_questions[:7]})
    template = 'mobile/404.html' if request.is_mobile else '404.html'
    data = {'message': 'Sorry, Question does not exists',
            'reason': "The URL may be misspelled or the language you're looking for is no longer available."}
...
def assessments_questions(request, **kwargs):
    ...
    latest_jobs = JobPost.objects.filter(status='Live').exclude(
        job_type='walk-in').select_related('company').prefetch_related('location', 'skills')[:7]
    ...
    if meta:
        ...
        meta_description = Template(meta['meta_description']).render(Context({}))
        h1_tag = Template(meta['h1_tag']).render(Context({}))
    return render(request, 'assessments/questions_list.html', {'questions': questions,
                                                               'aft_page': aft_page,
                                                               'after_page': after_page,
                                                               'prev_page': prev_page,
                                                               'previous_page': previous_page,
    ...
    if request.POST.get('mode') == 'add_like':
        ...
        if model == 'question':
            like = AssessmentData.objects.filter(
                question=Question.objects.get(id=int(obj_id)), user=user, comment='').first()
            if like:
                like.like = True
                like.dislike = False
                like.save()
            else:
                AssessmentData.objects.create(question=Question.objects.get(id=int(obj_id)), user=user, like=True)
        else:
            like = AssessmentData.objects.filter(
                solution=Solution.objects.get(id=int(obj_id)), user=user, comment='').first()
            if like:
                like.like = True
                like.dislike = False
                like.save()
            else:
                AssessmentData.objects.create(solution=Solution.objects.get(id=int(obj_id)), like=True, user=user)
        return HttpResponse(json.dumps({'error': False}))
    if request.POST.get('mode') == 'dis_like':
        ...
        if model == 'question':
            ...
            if dislike:
                ...
                AssessmentData.objects.create(question=Question.objects.get(id=int(obj_id)), user=user, dislike=True)
        else:
            dislike = AssessmentData.objects.filter(
                solution=Solution.objects.get(id=int(obj_id)), user=user, comment='').first()
            if dislike:
                ...
                AssessmentData.objects.create(solution=Solution.objects.get(id=int(obj_id)), dislike=True, user=user)
        return HttpResponse(json.dumps({'error': False}))
        ...
        if request.POST.get('comment'):
            ...
            if model == 'question':
                AssessmentData.objects.create(
                    question=Question.objects.get(id=int(obj_id)), user=user, comment=request.POST.get('comment'))
            else:
                AssessmentData.objects.create(
                    solution=Solution.objects.get(id=int(obj_id)), user=user, comment=request.POST.get('comment'))
            return HttpResponse(json.dumps({'error': False, 'comment': request.POST.get('comment')}))
        else:
            return HttpResponse(json.dumps({'error': True, 'message': 'Please enter your comment'}))
    return HttpResponse(json.dumps({'error': True}))


def get_messages(request):
    if request.POST.get('job_id'):
        db.messages.update({'$and': [{'message_to': request.user.id}, {'job_id': int(request.POST.get('job_id'))}]},
                           {'$set': {'is_read': True}}, multi=True)
        messages = db.messages.find({'$and': [{'$or': [{'message_from': request.user.id}, {'message_to': request.user.id}]},
                                              {'job_id': int(request.POST.get('job_id'))}]})
    else:
        db.messages.update({'$and': [{'message_to': request.user.id}, {'message_from': int(request.POST.get('r_id'))}]},
                           {'$set': {'is_read': True}}, multi=True)
        messages = db.messages.find({'$or': [{'$and': [{'message_from': request.user.id}, {'message_to': int(request.POST.get('r_id'))},
                                                       {'job_id': None}]},
                                             {'$and': [{'message_to': request.user.id}, {'message_from': int(request.POST.get('r_id'))},
                                                       {'job_id': None}]}
                                             ]})
    ...
    if user:
        messages = render_to_string("candidate/messages.html", {'messages': list(messages), 'user': user, 'job': job,
...
def messages(request):
    ...
    if request.POST.get('post_message'):
        data = {'message': request.POST.get('message'), 'message_from': request.user.id, 'message_to': int(request.POST.get('message_to')),
                'created_on': datetime.datetime.now(), 'is_read': False}
        if request.POST.get('job_id'):
            data['job_id'] = int(request.POST.get('job_id'))
        msg = db.messages.insert(data)
        time = datetime.datetime.now().strftime('%b. %d, %Y, %l:%M %p')
        return HttpResponse(json.dumps({'error': False, 'message': request.POST.get('message'), 'job_post': request.POST.get('job_id'),
                                        'msg_id': str(msg), 'time': time}))
    ...
    if request.user.is_authenticated:
        messages = list(db.messages.find({'message_to': request.user.id}, {'job_id': 1, 'message_from': 1, '_id': 0}).sort([("created_on", -1)]))
        ...
        if users.exists():
            recruiter_order = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(recruiter_ids)])
            users = users.order_by(recruiter_order)
        template = 'mobile/profile/profile.html' if request.is_mobile else 'candidate/user_messages.html'
