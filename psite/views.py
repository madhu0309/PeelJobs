...
def pages(request, page_name):
    pages_slugs = ['about-us', 'terms-conditions',
                   'privacy-policy', 'recruiter-faq', 'faq']
    ...
    if request.method == "POST":
        ...
        if validate_simplecontactform.is_valid():
            ...
            if json.loads(r.text)['success']:
                ...
                data = {
                    'error': False, 'response': 'Thanks for contacting Us, we will reach you soon!'}
                return HttpResponse(json.dumps(data))
            data = {'error': True, 'captcha_response': 'Choose Correct Captcha'}
            return HttpResponse(json.dumps(data))
        data = {
            'error': True, 'response': validate_simplecontactform.errors}
        return HttpResponse(json.dumps(data))
    if request.is_mobile:
        return render(request, 'mobile/contact-us.html', {'enquery_types': ENQUERY_TYPES})
...
def sitemap(request, **kwargs):
    ...
    return render(request, template, {'jobposts': jobposts,
                                      'aft_page': aft_page,
...
def auth_return(request):
    ...
    token_valid = xsrfutil.validate_token(settings.SECRET_KEY, bytearray(state, 'utf-8'), request.user)
