...
from peeldb.models import (Country, JobPost, State, City, Skill, Industry, Qualification, AppliedJobs, Twitter, User, JOB_TYPE,
                           FacebookPost, TwitterPost, FunctionalArea, Keyword, UserEmail, MARTIAL_STATUS, Google, Facebook, Linkedin,
                           Company, MailTemplate, SentMail, InterviewLocation, COMPANY_TYPES, Menu, Ticket,
                           AGENCY_INVOICE_TYPE, AGENCY_JOB_TYPE, AgencyCompany, AgencyRecruiterJobposts, AGENCY_RECRUITER_JOB_TYPE,
                           AgencyApplicants, AgencyResume, POST)
from .forms import (JobPostForm, YEARS, MONTHS, Company_Form, User_Form, ChangePasswordForm, PersonalInfoForm, MobileVerifyForm,
                    MailTemplateForm, EditCompanyForm, RecruiterForm, MenuForm, ApplicantResumeForm, ResumeUploadForm)
from .tasks import (del_jobpost_tw, del_jobpost_fb, del_jobpost_peel_fb, add_twitter_friends_followers, add_google_friends,
                    add_facebook_friends_pages_groups)
from dashboard.tasks import sending_mail
from mpcomp.views import (rand_string, Memail, recruiter_login_required, get_prev_after_pages_count, agency_admin_login_required,
                          get_next_month, get_aws_file_path, mongoconnection, get_resume_data, handle_uploaded_file)
...
def jobs_list(request):
    if request.user.agency_admin:
        active_jobs_list = JobPost.objects.filter(user__company=request.user.company).exclude(
            status='Disabled').exclude(status='Expired').prefetch_related(
            'location', 'agency_recruiters').annotate(responses=Count('appliedjobs')).order_by('-id')
    elif request.user.is_agency_recruiter:
        active_jobs_list = JobPost.objects.filter(Q(agency_recruiters__in=[request.user]) | Q(user=request.user)).exclude(
            status='Disabled').exclude(status='Expired').prefetch_related(
            'location', 'agency_recruiters').annotate(responses=Count('appliedjobs')).order_by('-id').distinct()
    else:
    ...
    return render(request, 'recruiter/job/list.html', {'jobs_list': active_jobs_list,
                                                       'aft_page': aft_page,
...
def inactive_jobs(request):
    inactive_jobs_list = JobPost.objects.filter(Q(user=request.user.id) & Q(
        status='Disabled') | Q(status='Expired')).order_by('-id')
    ...
    return render(request, 'recruiter/job/list.html', {'jobs_list': inactive_jobs_list,
                                                       'aft_page': aft_page,
                                                       'after_page': after_page,
                                                       'prev_page': prev_page,
                                                       'previous_page': previous_page,
                ...
                if value != '':
                    ...
                    if skills:
                        ...
                        skill = Skill.objects.create(
                            name=value, status='InActive', slug=slugify(value), skill_type='Technical')
                        c = {'job_post': job_post, 'user': user, 'item': value, 'type': 'Skill', 'value': skill.name}
            ...
            for value in other_skills:
                if value != '':
                    ...
                    if qualification:
                        ...
                        qualification = Qualification.objects.create(name=value, status='InActive', slug=slugify(value))
                        job_post.edu_qualification.add(qualification)
                        c = {'job_post': job_post, 'user': user, 'item': value, 'type': 'Qualification', 'value': qualification.name}
                        rendered = temp.render(c)
                        Memail(mto, mfrom, subject, rendered, True)
            ...
            for value in o_industries:
                if value != '':
                    ...
                    if industry:
                        ...
                        c = {'job_post': job_post, 'user': user, 'item': value, 'type': 'Industry', 'value': industry.name}
                        rendered = temp.render(c)
                        Memail(mto, mfrom, subject, rendered, True)
            ...
            for value in other_areas:
                if value != '':
                    ...
                    if functional_area:
                        ...
                        c = {'job_post': job_post, 'user': user, 'item': value, 'type': 'Functional Area', 'value': functional_area.name}
                        rendered = temp.render(c)
                        Memail(mto, mfrom, subject, rendered, True)
        ...
        for location in locations:
            ...
            if locations:
                ...
                location = City.objects.create(
                    name=location, status='Disabled', slug=slugify(location), state=State.objects.get(id=16))
                post.location.add(location)
                c = {'job_post': post, 'user': user, 'item': 'Location', 'type': 'Location', 'value': location.name}
                rendered = temp.render(c)
                Memail(mto, mfrom, subject, rendered, True)
...
def set_other_fields(post, data, user):
    if data.get('last_date'):
        last_date = datetime.strptime(
            data.get('last_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
    ...
    if data.get('job_type') == 'walk-in':
        post.vacancies = 0
        post.walkin_contactinfo = data.get(
            'walkin_contactinfo')
        walkin_from_date = datetime.strptime(
            data.get('walkin_from_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
        ...
        walkin_to_date = datetime.strptime(
            data.get('walkin_to_date'), "%m/%d/%Y").strftime("%Y-%m-%d")
    ...
    if user.agency_admin or user.has_perm('jobposts_edit'):
        ...
        post.agency_invoice_type = data.get(
            'agency_invoice_type')
        if data.get('agency_client'):
            client = AgencyCompany.objects.get(
                id=data.get('agency_client'))
...
def adding_other_fields_data(data, post, user):
    ...
    if 'final_edu_qualification' in data.keys():
        add_other_qualifications(post, json.loads(data['final_edu_qualification']), user)
    ...
    if request.method == 'GET':
        if request.GET.get('q'):
            companies = Company.objects.filter(
                name__icontains=request.GET.get('q'), is_active=True).distinct()[:10]
            companies_names = []
        ...
        if request.user.is_active:
            if request.user.is_company_recruiter and not request.user.is_admin and not request.user.has_perm('jobposts_edit'):
            ...
            functional_area = FunctionalArea.objects.all().exclude(status='InActive').order_by('name')
            industries = Industry.objects.all().exclude(status='InActive').order_by('name')
            qualifications = Qualification.objects.all().exclude(status='InActive').order_by('name')
            ...
            if request.user.agency_admin or request.user.has_perm('jobposts_edit'):
                jobposts = JobPost.objects.filter(job_type=status, user__company=request.user.company)
            else:
            ...
            return render(request, 'recruiter/job/new.html', {'job_types': JOB_TYPE,
...
def edit_job(request, job_post_id):
    if request.user.agency_admin or request.user.has_perm('jobposts_edit'):
        job_post = JobPost.objects.filter(id=job_post_id, user__company=request.user.company).first()
    else:
    ...
    if request.method == 'GET':
        if request.user.mobile_verified:
            if job_post:
                ...
                qualifications = list(Qualification.objects.filter(status='Active').order_by('name'))
                qualifications.extend(job_post.edu_qualification.filter(status='InActive'))

                recruiters = User.objects.filter(company=request.user.company)
                clients = AgencyCompany.objects.filter(company=request.user.company)

                functional_area = list(FunctionalArea.objects.filter(status='Active').order_by('name'))
                ...
                return render(request, 'recruiter/job/edit.html', {'fb_groups': fb_groups,
                                                                   'job_types': JOB_TYPE,
                                                                   'qualifications': qualifications,
                                                                   'functional_area': functional_area,
        ...
        return render(request, 'recruiter/recruiter_404.html', {'message': message, 'reason': reason}, status=404)

    validate_form = JobPostForm(request.POST, request.FILES, instance=job_post, user=request.user)
    errors = retreving_form_errors(request, validate_form)
    if not errors:
        validate_post = validate_form.save(commit=False)
        save_job_post(validate_post, request)
        ...
        adding_other_fields_data(request.POST, validate_post, request.user)
        data = {'error': False, 'response': 'Jobpost Updated Successfully',
...
def copy_job(request, status):
    if request.user.agency_admin or request.user.has_perm('jobposts_edit'):
        jobposts = JobPost.objects.filter(job_type=status, user__company=request.user.company)
        job_post = JobPost.objects.filter(id=request.GET.get('jobpost_id'), user__company=request.user.company).first()
    else:
        jobposts = JobPost.objects.filter(job_type=status, user=request.user)
        job_post = JobPost.objects.filter(id=request.GET.get('jobpost_id'), user=request.user).first()
    ...
    if request.method == 'GET':
        if request.user.mobile_verified:
            if job_post:
                ...
                functional_area = list(FunctionalArea.objects.all().exclude(status='InActive').order_by('name'))
                functional_area.extend(job_post.functional_area.filter(status='InActive'))

                return render(request, 'recruiter/job/copy.html', {'job_types': JOB_TYPE,
                                                                   'qualifications': qualifications,
                                                                   'functional_area': functional_area,
                                                                   'years': YEARS, 'months': MONTHS,
                                                                   'job_post': job_post,
            ...
            message = 'Sorry, No Job Posts Found'
            reason = "The URL may be misspelled or the job you're looking for is no longer available."
            return render(request, 'recruiter/recruiter_404.html', {'message_type': '404',
                                                                    'message': message,
                                                                    'reason': reason}, status=404)
        message = 'Sorry, Your mobile number is not verified'
        reason = "Please verify your mobile number"
...
def view_job(request, job_post_id):
    if request.POST.get('post_message'):
        data = {'message': request.POST.get('message'), 'message_from': request.user.id, 'message_to': int(request.POST.get('message_to')),
                'created_on': datetime.now(), 'job_id': int(request.POST.get('job_id')), 'is_read': False}
        msg_id = db.messages.insert(data)
        time = datetime.now().strftime('%b. %d, %Y, %l:%M %p')
        return HttpResponse(json.dumps({'error': False, 'message': request.POST.get('message'), 'msg_id': str(msg_id), 'time': time}))
    if request.POST.get('get_applicant'):
        if request.POST.get('user_type') == 'resume_pool':
            ...
            applicant = render_to_string("recruiter/job/resume_applicant.html", {"user": user})
        else:
            user = User.objects.filter(id=request.POST.get('user_id')).first()
            db.messages.update({'$and': [{'message_to': request.user.id}, {'message_from': user.id}, {'job_id': int(job_post_id)}]},
                               {'$set': {'is_read': True}}, multi=True)
            messages = db.messages.find({'$or': [{'$and': [{'message_from': user.id}, {'message_to': request.user.id}, {'job_id': int(job_post_id)}]},
                                        {'$and': [{'message_to': user.id}, {'message_from': request.user.id}, {'job_id': int(job_post_id)}]}]})
            try:
            ...
            applicant = render_to_string("recruiter/job/applicant_profile.html", {"user": user,
                                                                                  'messages': list(messages),
                                                                                  'job_id': job_post_id,
                                                                                  'user_pic': user_pic,
                                                                                  'profile_pic': profile_pic}, request)
        if user:
            return HttpResponse(json.dumps({'error': False, 'profile': applicant}))
        return HttpResponse(json.dumps({'error': True, 'response': 'Profile Not Found!'}))
    if request.user.agency_admin:
        jobposts = JobPost.objects.filter(id=job_post_id, user__company=request.user.company)
    elif request.user.is_agency_recruiter:
        jobposts = JobPost.objects.filter(Q(user=request.user) | Q(agency_recruiters__in=[request.user]))
    else:
        jobposts = JobPost.objects.filter(id=job_post_id, user=request.user)
    if jobposts:
        jobpost = jobposts[0]
        all_applicants = AppliedJobs.objects.filter(job_post=jobpost).prefetch_related('user', 'resume_applicant').distinct()
        ...
        if request.POST.get('search_value'):
            ...
            if request.POST.get('search_value') == 'resume_pool':
                ...
                shortlisted_applicants = shortlisted_applicants.exclude(resume_applicant=None)
        ...
        if search_location or search_skills:
            all_applicants = all_applicants.filter(Q(user__current_city__id__in=search_location) |
                                                   Q(user__skills__skill__id__in=search_skills))
            shortlisted_applicants = shortlisted_applicants.filter(Q(user__current_city__id__in=search_location) |
                                                                   Q(user__skills__skill__id__in=search_skills))
            rejected_applicants = rejected_applicants.filter(Q(user__current_city__id__in=search_location) |
                                                             Q(user__skills__skill__id__in=search_skills))
            selected_applicants = selected_applicants.filter(Q(user__current_city__id__in=search_location) |
                                                             Q(user__skills__skill__id__in=search_skills))
            process_applicants = process_applicants.filter(Q(user__current_city__id__in=search_location) |
                                                           Q(user__skills__skill__id__in=search_skills))
            pending_applicants = pending_applicants.filter(Q(user__current_city__id__in=search_location) |
                                                           Q(user__skills__skill__id__in=search_skills))
        # if request.user.agency_admin:
        ...
        agency_resumes = AgencyResume.objects.filter(uploaded_by=request.user).exclude(id__in=ids)
        if request.POST.get('search_value') == 'users':
            agency_resumes = []
        if request.POST.getlist('applicants'):
            ...
            if validate_resume_applicant.is_valid():
                for resume in request.POST.getlist('applicants'):
                    applied_jobs = AppliedJobs.objects.filter(resume_applicant__id=resume, job_post__id=request.POST.get('job_post'))
                    if not applied_jobs:
                        AppliedJobs.objects.create(status=request.POST.get('status'), resume_applicant_id=resume, job_post_id=request.POST.get('job_post'),
        ...
        if meta:
            meta_title = Template(meta.get('meta_title')).render(Context({'job': jobpost}))
            meta_description = Template(meta.get('meta_description')).render(Context({'job': jobpost}))
        return render(request, 'recruiter/job/view.html', {'jobpost': jobpost,
        ...
        reason = "The URL may be misspelled or the job you're looking for is no longer available."
        return render(request, 'recruiter/recruiter_404.html', {'message': 'Sorry, No Job Posts Found',
                                                                'reason': reason}, status=404)
...
def applicants(request, job_post_id):
    ...
    if jobpost:
        ...
        if request.POST.get('type') == 'resume':
            user = AppliedJobs.objects.filter(
                resume_applicant_id=user_id, job_post_id=job_post_id
            ).prefetch_related('resume_applicant').first()
        else:
            user = AppliedJobs.objects.filter(
                user_id=user_id, job_post_id=job_post_id
        ...
        if request.POST.get('type') == 'resume':
            ...
            mto = [user.user.email]
        if request.POST.get('type') == 'resume':
            names_dict = {'job_post': jobpost[0], 'user_email': user.resume_applicant.email,
                          'user_status': user.status, 'name': user.resume_applicant.candidate_name}
        else:
            names_dict = {'job_post': jobpost[0], 'user_email': user.user.email,
                          'user_status': user.status, 'name': user.user.get_full_name()}
        rendered = temp.render(names_dict)
        Memail(mto, settings.DEFAULT_FROM_EMAIL, subject, rendered, False)
        data = {'error': False, 'response': 'Applicant Status changed to ' + user.status, 'prev_count': prev_count,
                'next_count': next_count, 'prev_status': prev_status, 'next_status': user.status}
        return HttpResponse(json.dumps(data))
    return HttpResponse(json.dumps({'error': True, 'response': 'Something went wrong!'}))


@recruiter_login_required
def preview_job(request, job_post_id):
    if request.user.is_agency_recruiter:
        job_post = JobPost.objects.filter(id=job_post_id, user__company=request.user.company)
        if not request.user.agency_admin:
            job_post = job_post.filter(Q(user=request.user) | Q(agency_recruiters__in=[request.user]))
    else:
    ...
    reason = "The URL may be misspelled or the job you're looking for is no longer available."
    return render(request, 'recruiter/recruiter_404.html', {'message_type': message_type, 'message': message, 'reason': reason}, status=404)
...
def get_autocomplete(request):
    companies = Company.objects.filter(name__icontains=request.GET.get('q'), company_type='Company',
                                       id__in=User.objects.filter(is_admin=False).values_list('company', flat=True)).distinct()[:10]
    companies_names = []
    for each in companies:
        companies_names.append(each.name)
    if request.GET.get('register_name'):
...
def new_user(request):  # pragma: no mccabe
    if request.method == 'GET':
        if request.GET.get('q'):
            ...
            return render(request, 'recruiter/register.html', {'meta_title': meta_title,
                                                               'meta_description': meta_description,
                                                               'h1_tag': h1_tag})
    ...
    if request.method == 'POST':
        ...
        if show_errors:
            ...
            while response == '':
                try:
                    response = requests.get('https://www.google.com/recaptcha/api/siteverify', params=payload)
                except:
                    time.sleep(5)
            if json.loads(response.text)['success']:
                if request.POST.get('client_type') == "company":
                    if company_form.is_valid():
                        if companies:
                            ...
                            each_company = Company.objects.create(name=request.POST['name'], website=request.POST['website'],
                                                                  company_type='Consultant', slug=slugify(request.POST['name']),
                                                                  email=request.POST.get('email'), created_from='register')
                    else:
                ...
                if user_obj.is_valid():
                    ...
                    user_obj = User.objects.create(first_name=username, email=request.POST[
                                                   'email'], username=username, mobile=request.POST['mobile'], profile_updated=datetime.now(timezone.utc))
                    ...
                    while True:
                        ...
                        u = User.objects.filter(
                            activation_code__iexact=random_code)
                        if not u:
                            break
                    while True:
                    ...
                    if 'client_type' in request.POST and request.POST['client_type'] == "company":
                        url = request.scheme + '://' + \
                            request.META[
                                'HTTP_HOST'] + "/agency/activation/" + str(user_obj.activation_code) + "/"
                    else:
                        url = request.scheme + '://' + \
                            request.META[
    ...
    if request.method == 'POST':
        if request.POST.get('email'):
            ...
            if user and user[0].is_jobseeker:
                data = {'error': True, 'email': 'User Already registered as a Applicant'}
                return HttpResponse(json.dumps(data))
            ...
            if user:
                ...
                if not usr.is_active:
                    if usr.company:
                        ...
                        url = request.scheme + '://' + \
                            request.META['HTTP_HOST'] + "/recruiter/activation/" + str(usr.activation_code) + "/"
                c = {'randpwd': randpwd, 'user': usr, 'redirect_url': url, 'activate_url': url}
                rendered = temp.render(c)
                ...
                data = {'error': False, 'info': 'Sent a link to your email, reset your password by clicking that link' if usr.is_active else
                                        'An email has been sent to your email id, Please activate your account'}
                return HttpResponse(json.dumps(data))
            else:
                data = {
...
def change_password(request):
    if request.method == 'POST':
        validate_changepassword = ChangePasswordForm(
...
def send_mobile_verification_code(request):
    ...
    if request.method == 'POST':
        ...
        user.save()
        return HttpResponse(json.dumps({'error': False, 'message': 'An OTP sent to your mobile successfully'}))
    ...
    if request.method == 'POST':
        ...
        if 'profile_pic' in request.FILES:
            ...
            if str(ftype) in sup_formates:
                ...
                return HttpResponse(json.dumps(data))
            data = {"error": True, 'data': 'Upload a valid Image format, Ex: PNG, JPEG, JPG'}
            return HttpResponse(json.dumps(data))
        data = {'error': True, 'data': 'Please Upload Profile Pic'}
        return HttpResponse(json.dumps(data))
    user = User.objects.filter(id=request.user.id).prefetch_related('technical_skills', 'industry', 'functional_area')
    return render(request, 'recruiter/user/profile.html', {'countries': countries, 'user': user[0]})
...
def index(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            if user.is_jobseeker or user.is_staff:
                data = {'error': True,
                        'message': "You have registered as " + ("Job Seeker" if user.is_jobseeker else "Admin") + " and you can't login as Employer"}
                return HttpResponse(json.dumps(data))
            ...
            if user.is_active:
                ...
                if not user.mobile_verified:
                    ...
                    if password_reset_diff > 600:
                        ...
                        requests.get('http://sms.9sm.in/rest/services/sendSMS/sendGroupSms?AUTH_KEY='+str(settings.SMS_AUTH_KEY)+'&message=' + str(
                            message) + '&senderId='+str(settings.BULK_SMS_FROM)+'&routeId=1&mobileNos=' + str(user.mobile) + '&smsContentType=english')
                ...
                if user.company:
                    url = request.scheme + '://' + \
                        ...
                        "/agency/activation/" + str(user.activation_code) + "/"
                else:
                    url = request.scheme + '://' + \
                        request.META[
    ...
    if meta:
        ...
        meta_description = Template(meta[0]['meta_description']).render(Context({}))
        h1_tag = Template(meta[0]['h1_tag']).render(Context({}))
    return render(request, 'recruiter/login.html', {'meta_title': meta_title,
                                                    'meta_description': meta_description,
                                                    'h1_tag': h1_tag})
...
def edit_profile(request):
    if request.method == 'GET':
        ...
        user = User.objects.filter(id=request.user.id).prefetch_related('technical_skills', 'industry', 'functional_area')
        return render(request, 'recruiter/user/details.html', {'skills': Skill.objects.all(),
                                                               ...
                                                               'user': user[0]})
    validate_user = PersonalInfoForm(
    ...
    if validate_user.is_valid():
        ...
        if request.POST.get('dob'):
            dob = datetime.strptime(
                request.POST.get('dob'), "%m/%d/%Y").strftime("%Y-%m-%d")
            user.dob = dob
        else:
            user.dob = None
        ...
        user_login = False
        password_reset_diff = int((datetime.now() - user.last_mobile_code_verified_on).seconds)
        if not user.mobile_verified:
            if password_reset_diff > 600:
                random_code = rand_string(size=6)
                message = 'Hello ' + request.user.username + ', An OTP ' + random_code + \
                ...
                # requests.get("http://182.18.160.225/index.php/api/bulk-sms", params=data)
                requests.get('http://sms.9sm.in/rest/services/sendSMS/sendGroupSms?AUTH_KEY='+str(settings.SMS_AUTH_KEY)+'&message=' + str(
                    message) + '&senderId='+str(settings.BULK_SMS_FROM)+'&routeId=1&mobileNos=' + str(request.POST.get('mobile')) + '&smsContentType=english')
...
def twitter_login(request):
    ...
    if 'oauth_verifier' in request.GET:
        ...
        twitter = Twython(TW_APP_KEY, TW_APP_SECURE, request.session[
                          'OAUTH_TOKEN'], request.session['OAUTH_TOKEN_SECRET'])
        final_step = twitter.get_authorized_tokens(oauth_verifier)
        if final_step.get('oauth_token_secret'):
            twitter = Twython(TW_APP_KEY, TW_APP_SECURE, final_step[
                              'oauth_token'], final_step['oauth_token_secret'])
            ...
            add_twitter_friends_followers.delay(
                request.user.id, friends, followers)
            return HttpResponseRedirect(reverse('recruiter:index'))
        message_type = 'Sorry,'
        ...
        return render(request, 'recruiter/recruiter_404.html', {'message_type': message_type,
                                                                'message': message,
                                                                'reason': reason,
                                                                'email': email,
                                                                'number': number}, status=404)
    else:
...
def google_login(request):
    if 'code' in request.GET:
        params = {
            ...
            'redirect_uri': request.scheme + '://' + request.META['HTTP_HOST'] + reverse('recruiter:google_login'),
        ...
        if not info.get('access_token'):
            return render(request, '404.html', {'message_type': 'Sorry,',
        ...
        if email_matches:
            ...
            if user.is_recruiter or user.is_agency_recruiter:
                ...
                return HttpResponseRedirect(reverse('recruiter:new_user') + '?invalid=Only Recruiters and Agencies are allowed to log in.')
        else:
            return HttpResponseRedirect(reverse('recruiter:new_user') + '?invalid=Sorry, User not found with this mail.')
    else:
        rty = "https://accounts.google.com/o/oauth2/auth?client_id=" + \
            settings.GP_CLIENT_ID + "&response_type=code&scope="
        rty += "https://www.googleapis.com/auth/userinfo.profile https://www.googleapis.com/auth/userinfo.email " + \
               "https://www.googleapis.com/auth/contacts.readonly"
        rty += "&redirect_uri=" + request.scheme + '://' + \
            request.META[
                'HTTP_HOST'] + reverse('recruiter:google_login') + "&state=1235dfghjkf123"
...
def google_connect(request):
    if 'code' in request.GET:
        ...
        params = {
            'grant_type': 'authorization_code',
            'code': code,
            'redirect_uri': request.scheme + '://' + request.META['HTTP_HOST'] + reverse('recruiter:google_connect'),
            'client_id': settings.GP_CLIENT_ID,
            'client_secret': settings.GP_CLIENT_SECRET,
        }
        info = requests.post("https://accounts.google.com/o/oauth2/token", data=params)
        ...
        rty += "&redirect_uri=" + request.scheme + '://' + \
            request.META[
                'HTTP_HOST'] + reverse('recruiter:google_connect') + "&state=1235dfghjkf123"
        return HttpResponseRedirect(rty)
...
def facebook_login(request):
    if 'code' in request.GET:
        accesstoken = get_access_token_from_code(request.GET['code'], request.scheme + '://' + request.META[
                                                 'HTTP_HOST'] + reverse('recruiter:facebook_login'), settings.FB_APP_ID, settings.FB_SECRET)
        if accesstoken.get('access_token'):
            ...
            profile = graph.get_object("me", fields="id, name, email, birthday, hometown, location, link, locale, gender, timezone")
            # print profile
            ...
            if profile.get('email'):
                ...
                bday = datetime.strptime(profile['birthday'], '%m/%d/%Y').strftime(
                    '%Y-%m-%d') if profile.get('birthday') else '1970-09-09'
                ...
                add_facebook_friends_pages_groups(accesstoken, profile['id'], request.user)
                return HttpResponseRedirect(reverse('recruiter:index'))
        message_type = 'Sorry,'
        ...
        # our code to fix it
        rty = "https://graph.facebook.com/oauth/authorize?client_id=" + settings.FB_APP_ID + "&redirect_uri=" + request.scheme + \
            '://' + request.META['HTTP_HOST'] + reverse('recruiter:facebook_login') + \
...
def linkedin_login(request):
    if 'code' in request.GET:
        ...
        params['redirect_uri'] = request.scheme + '://' + request.META['HTTP_HOST'] + reverse('recruiter:linkedin_login')
        ...
        args = {
            "grant_type": 'authorization_code',
            "code": request.GET.get('code'),
            "redirect_uri": request.scheme + '://' + request.META['HTTP_HOST'] + reverse('recruiter:linkedin_login'),
            "client_id": settings.LN_API_KEY,
            "client_secret": settings.LN_SECRET_KEY
        }
        response = requests.get("https://www.linkedin.com/uas/oauth2/accessToken?" + urllib.parse.urlencode(args)).json()
        if response.get('access_token'):
            ...
            rty = "https://api.linkedin.com/v1/people/~:(" + required_info + ")" + \
        ...
        rty += "&redirect_uri=" + request.scheme + '://' + \
            request.META['HTTP_HOST'] + reverse('recruiter:linkedin_login')
        return HttpResponseRedirect(rty)
...
def new_template(request, jobpost_id):
    if request.method == "POST":
        ...
        if validate_mailtemplate.is_valid():
            MailTemplate.objects.create(title=request.POST.get('title'), subject=request.POST.get('subject'), message=request.POST.get(
                'message'), created_on=datetime.utcnow(), modified_on=datetime.utcnow(), created_by=request.user, job_post_id=jobpost_id)
        ...
        return HttpResponse(json.dumps(data))
    return render(request, 'recruiter/mail/new_mailtemplate.html', {'jobpost_id': jobpost_id})
...
def edit_template(request, jobpost_id, template_id):
    ...
    if mailtemplates:
        ...
        if request.method == "POST":
            ...
            data = {'error': True, 'message': validate_mailtemplate.errors}
            return HttpResponse(json.dumps(data))
        return render(request, 'recruiter/mail/edit_mailtemplate.html', {'email_template': mailtemplate, 'jobpost_id': jobpost_id})
    message = 'Sorry, the page you requested can not be found'
    reason = "The URL may be misspelled or the page you're looking for is no longer available."
    return render(request, 'recruiter/404.html', {'message_type': '404',
                                                  'message': message, 'reason': reason}, status=404)
...
def emailtemplates(request, jobpost_id):
    ...
    return render(request, 'recruiter/mail/list.html', {"mailtemplates": mailtemplates, 'jobpost_id': jobpost_id, 'jobpost': jobpost})


@recruiter_login_required
def view_template(request, template_id):
    mailtemplate = MailTemplate.objects.filter(
        id=template_id, created_by=request.user).first()
    if mailtemplate:
        return render(request, 'recruiter/mail/view.html', {"mailtemplate": mailtemplate})
    reason = "The URL may be misspelled or the page you're looking for is no longer available."
    return render(request, 'recruiter/404.html', {'message_type': '404',
                                                  'message': 'Sorry, the page you requested can not be found',
                                                  'reason': reason}, status=404)
    ...
    if mailtemplates:
        ...
        if request.method == 'POST':
            ...
            if validate_mailtemplate.is_valid():
                ...
                Memail(mto, settings.DEFAULT_FROM_EMAIL, emailtemplate.subject, rendered, False)
                sending_mail.delay(
                    emailtemplate, request.POST.getlist('recruiters'))
                data = {'error': False, 'response': 'Email Sent Successfully'}
                return HttpResponse(json.dumps(data))
            data = {
                'error': True, 'response': validate_mailtemplate.errors}
            return HttpResponse(json.dumps(data))
        applicants = AppliedJobs.objects.filter(job_post_id=jobpost_id)
        return render(request, 'recruiter/mail/send_mail.html', {'applicants': applicants, 'mailtemplate': emailtemplate, 'jobpost_id': jobpost_id})
    else:
        reason = "The URL may be misspelled or the page you're looking for is no longer available."
        return render(request, 'recruiter/404.html', {'message_type': '404',
                                                      'message': 'Sorry, the page you requested can not be found',
...
def sent_mails(request, jobpost_id):
    ...
    return render(request, 'recruiter/mail/sent_mail_list.html', {"sent_mails": sent_mails, 'jobpost_id': jobpost_id})
...
def enable_email_notifications(request, job_post_id):
    ...
    job_post.save()
    return HttpResponseRedirect(reverse('recruiter:view', kwargs={'job_post_id': job_post_id}))


def view_sent_mail(request, sent_mail_id):
    sent_mail = SentMail.objects.filter(id=sent_mail_id).first()
    if sent_mail:
        return render(request, 'recruiter/mail/view_sent_mail.html', {"sent_mail": sent_mail})
    reason = "The URL may be misspelled or the page you're looking for is no longer available."
    return render(request, 'recruiter/404.html', {'message_type': '404',
                                                  'message': 'Sorry, the page you requested can not be found',
                                                  'reason': reason}, status=404)
...
def interview_location(request, location_count):
    ...
    return render(request, "recruiter/job/add_interview_location.html",
...
def view_company(request):
    ...
    return render(request, 'recruiter/company/view_microsite_page.html',
...
def edit_company(request):
    ...
    if request.method == 'POST':
        if company:
            company_form = EditCompanyForm(request.POST, request.FILES, instance=company)
        else:
        ...
        if company_form.is_valid():
            ...
            if request.FILES.get('profile_pic'):
                ...
                file_path = get_aws_file_path(request.FILES.get('profile_pic'), 'company/logo/', slugify(request.POST['name']))
    ...
    if request.method == 'GET':
        if request.GET.get('q'):
            companies = Company.objects.filter(id__in=User.objects.filter(is_admin=False).values_list(
                'company', flat=True), name__icontains=request.GET.get('q')).distinct()
            companies_names = []
            for each in companies:
                companies_names.append(each.name)
            if request.GET.get('register_name'):
            ...
            if request.user.agency_admin or request.user.is_recruiter:
                ...
                template_name = 'recruiter/company/edit_microsite_page.html'
                data = {'message': "Sorry, You don't have permissions to access this page"}
                status = 404
        ...
        if 'profile_pic' in request.FILES:
            ...
            if str(ftype) in sup_formates:
                if request.user.company.profile_pic:
                    url = str(request.user.company.profile_pic).split('cdn.peeljobs.com')[-1:]
                    AWS().cloudfront_invalidate(paths=url)
                file_path = get_aws_file_path(logo, 'company/logo/', request.user.company.slug)
                request.user.company.profile_pic = file_path
                request.user.company.save()
                data = {"error": False, 'data': 'Company Profile Pic Uploaded Successfully'}
                return HttpResponse(json.dumps(data))
            else:
                data = {'error': True, 'data': 'Upload Valid Image Format Files Ex: Png, Jpg, JPEG'}
                return HttpResponse(json.dumps(data))
        else:
...
def company_recruiter_list(request):
    recruiters = User.objects.filter(
        company=request.user.company).exclude(id=request.user.id)
    if 'search' in request.GET.keys():
        ...
        if not (request.GET['search'].lower() == 'active' or str(request.GET['search'].lower()).replace(' ', '') == 'inactive'):
            recruiters = recruiters.filter(Q(first_name__icontains=request.GET['search']) | Q(
    ...
    search_value = request.GET[
        'search'] if 'search' in request.GET.keys() else ''
    return render(request, 'recruiter/company/recruiter_list.html', {'recruiters': recruiters,
                                                                     'aft_page': aft_page,
                                                                     'after_page': after_page,
                                                                     'prev_page': prev_page,
                                                                     'previous_page': previous_page,
...
def company_recruiter_create(request):
    ...
    permissions = Permission.objects.filter(
        content_type_id=contenttype, codename__icontains='jobposts').order_by('id')

    if request.method == 'POST':
        validate_recruiter = RecruiterForm(request.POST, request.FILES)
        if validate_recruiter.is_valid():
            user = User.objects.create(first_name=request.POST['first_name'], email=request.POST[
                                       'email'], username=request.POST['email'], job_role=request.POST['job_role'], mobile=request.POST['mobile'])
            if request.FILES.get('profile_pic'):
                user.profile_pic = request.FILES.get('profile_pic')
            ...
            user.save()
            if 'is_admin' in request.POST.keys() and request.POST.get('is_admin') == 'True':
                user.user_type = 'AA'
            ...
            return HttpResponse(json.dumps(data))
    return render(request, 'recruiter/company/create_recruiter.html', {'permissions': permissions})


@agency_admin_login_required
def edit_company_recruiter(request, recruiter_id):
    recruiters = User.objects.filter(
        company=request.user.company, id=recruiter_id).exclude(id=request.user.id)
    if recruiters:
        ...
        if request.method == 'POST':
            ...
            if validate_recruiter.is_valid():
                ...
                if recruiter.password != request.POST["password"] and not check_password(request.POST["password"], recruiter.password):
                ...
                data = {
                    'error': False, 'response': 'Recruiter Updated Successfully'}
                return HttpResponse(json.dumps(data))
            else:
                data = {'error': True, 'response': validate_recruiter.errors}
                return HttpResponse(json.dumps(data))
        return render(request, 'recruiter/company/create_recruiter.html', {'recruiter': recruiter,
...
def company_recruiter_profile(request, recruiter_id):
    ...
    if recruiters:
        ...
        if 'search_value' in request.GET.keys():
            if request.GET['search_value'] == 'all':
                ...
                job_posts = job_posts.filter(
                    job_type=request.GET['search_value'])
        if "page" in request.GET and int(request.GET.get('page')) > 0:
            page = int(request.GET.get('page'))
        else:
            page = 1
        ...
        search_value = request.GET.get(
            'search_value') if request.GET.get('search_value') else ''
        job_status = request.GET.get(
            'job_status') if request.GET.get('job_status') else 'active'

        return render(request, 'recruiter/company/recruiter_profile.html', {'recruiter': recruiter,
                                                                            'job_posts': job_posts,
                                                                            'tickets': tickets,
                                                                            'aft_page': aft_page,
                                                                            'after_page': after_page,
...
def messages(request):
    if request.GET.get('search'):
        user_ids = AppliedJobs.objects.filter(job_post__user__id=request.user.id).values_list('user', flat=True)
        users = User.objects.filter(Q(id__in=user_ids) & Q(Q(email__icontains=request.GET.get('search')) |
                                    Q(first_name__icontains=request.GET.get('search')) | Q(last_name__icontains=request.GET.get('search'))))
        return HttpResponse(json.dumps({'error': False, 'users': list(users.values_list('id', flat=True))}))
    if request.POST.get('mode') == 'get_messages':
        db.messages.update({'$and': [{'message_to': request.user.id}, {'message_from': int(request.POST.get('r_id'))}, {'job_id': None}]},
                           {'$set': {'is_read': True}}, multi=True)
        messages = db.messages.find({'$or': [{'$and': [{'message_from': request.user.id}, {'message_to': int(request.POST.get('r_id'))}, {'job_id': None}]},
                                             {'$and': [{'message_to': request.user.id}, {'message_from': int(request.POST.get('r_id'))}, {'job_id': None}]}
                                             ]})
        ...
        if user:
            ...
            if not profile_pic:
                profile_pic = 'https://cdn.peeljobs.com/dummy.jpg'
            messages = render_to_string("candidate/messages.html", {'messages': list(messages), 'user': user,
                                                                    'user_pic': user_pic, 'profile_pic': profile_pic}, request)
            return HttpResponse(json.dumps({'error': False, 'messages': messages}))
        else:
            return HttpResponse(json.dumps({'error': True, 'response': 'User Not Found!'}))
    if request.POST.get('post_message'):
        data = {'message': request.POST.get('message'), 'message_from': request.user.id, 'message_to': int(request.POST.get('message_to')),
                'created_on': datetime.now(), 'is_read': False}
    ...
    if request.POST.get('mode') == 'delete_message':
        ...
        if msg.get('message_from') == request.user.id or msg.get('message_to') == request.user.id:
    ...
    if request.POST.get('mode') == 'delete_chat':
        if request.POST.get('job'):
            messages = db.messages.remove({'$or': [{'$and': [{'message_from': request.user.id}, {'message_to': int(request.POST.get('user'))},
                                                             {'job_id': int(request.POST.get('job'))}]},
                                                   {'$and': [{'message_to': request.user.id}, {'message_from': int(request.POST.get('user'))},
                                                             {'job_id': int(request.POST.get('job'))}]}
                                                   ]})
        else:
            messages = db.messages.remove({'$or': [{'$and': [{'message_from': request.user.id}, {'message_to': int(request.POST.get('user'))},
        ...
        return HttpResponse(json.dumps({'error': False, 'message': request.POST.get('message')}))
    if request.user.is_authenticated:
        messages = list(db.messages.find({'$or': [{'message_from': request.user.id}, {'message_to': request.user.id}]},
                                                  {'message_from': 1, 'message_to': 1, '_id':0}).sort([("created_on", -1)]))
        user_ids = AppliedJobs.objects.filter(job_post__user__id=request.user.id).values_list('user', flat=True)
        users = User.objects.filter(id__in=user_ids)
        if messages:
            recruiter_ids = map(lambda d: d.get('message_from') if  d.get('message_to') == request.user.id else d.get('message_to'), messages)
            preserved = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(recruiter_ids)])
...
def resume_upload(request):
    if request.user.is_agency_admin:
        ...
        agency_jobposts = JobPost.objects.filter(Q(agency_recruiters__in=[request.user]) | Q(user=request.user)).distinct()
    if request.method == 'POST':
        resume_user = AgencyResume.objects.filter(email=request.POST.get('email'), uploaded_by=request.user).first()
        if not resume_user:
            validate_resume_upload = ResumeUploadForm(request.POST, request.FILES, request=request)
            if validate_resume_upload.is_valid():
                fo = request.FILES['resume']
                sup_formates = ["application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/pdf", "application/rtf",
                ...
                if str(ftype) in sup_formates:
                    if size < 300 and size > 0:
                        ...
                        path = 'resume/' + user_id + '/' + \
                            request.FILES['resume'].name.replace(" ", "-").encode('ascii', 'ignore').decode('ascii')
                        conn.upload(
                            path, request.FILES['resume'], settings.AWS_STORAGE_BUCKET_NAME, public=True)
                        email = request.POST.get('email')
                        user = User.objects.filter(email__iexact=email).first()
                        if not user:
                            user = User.objects.create(email=email, user_type='JS', username=email, mobile=request.POST.get('mobile', ''),
                                                       resume=path, registered_from='ResumePool')
                            passwd = get_random_string(length=10).lower()
                            user.set_password(passwd)
                            user.save()
                            save_codes_and_send_mail(user, request, passwd)
                        resume_upload = AgencyResume.objects.create(candidate_name=request.POST.get('candidate_name'), email=email, user=user,
                                                                    resume=path, uploaded_by=request.user, mobile=request.POST.get('mobile'))
                        ...
                        for job in request.POST.getlist('job_post'):
                            AppliedJobs.objects.create(status='Pending', resume_applicant=resume_upload, job_post_id=job,
                                                       ip_address=request.META['REMOTE_ADDR'], user_agent=request.META['HTTP_USER_AGENT'])
    ...
    return render(request, 'recruiter/company/resume_upload.html', {'skills': skills, 'agency_jobposts': agency_jobposts, 'status': POST})
        ...
        if str(resume.content_type) in sup_formates:
            if size < 300 and size > 0:
                ...
                if not email:
                    ...
                    return HttpResponse(json.dumps(data))
                resume_user = AgencyResume.objects.filter(email=email, uploaded_by=request.user).first()
                if resume_user:
                    ...
                    if replace and replace == 'true':
                        ...
                        random_string = ''.join(random.choice('0123456789ABCDEF') for i in range(3))
                        user_id = str(request.user.id) + str(random_string)
                        path = 'resume/' + user_id + '/' + resume.name.replace(" ", "-").encode('ascii', 'ignore').decode('ascii')
                        conn.upload(path, resume, settings.AWS_STORAGE_BUCKET_NAME, public=True)
                        resume_user.resume = path
                        resume_user.save()
                        data = {'duplicated': True, 'data': 'Resume Updated Successfully'}
                    elif replace:
                        data = {'duplicated': True, 'data': 'Resume Already Existed'}
                    else:
                        data = {'error': True, 'duplicate': True, 'email': email, 'fname': resume.name}
                    return HttpResponse(json.dumps(data))

                user = User.objects.filter(email__iexact=email).first()
                if not user:
                    user = User.objects.create(email=email, user_type='JS', username=email, registered_from='ResumePool')
                    passwd = get_random_string(length=10).lower()
                    user.set_password(passwd)
                    user.save()
                    save_codes_and_send_mail(user, request, passwd)
                ...
                path = 'resume/' + user_id + '/' + resume.name.replace(" ", "-").encode('ascii', 'ignore').decode('ascii')
                conn.upload(path, resume, settings.AWS_STORAGE_BUCKET_NAME, public=True)
                resume_upload = AgencyResume.objects.create(candidate_name=email, email=email, resume=path,
                                                            uploaded_by=request.user, mobile=mobile)
                for job in request.POST.getlist('job_post'):
                    AppliedJobs.objects.create(status='Pending', resume_applicant=resume_upload, job_post_id=job,
                                               ip_address=request.META['REMOTE_ADDR'], user_agent=request.META['HTTP_USER_AGENT'])
                data = {'error': False, 'data': 'Resume Uploaded Successfully'}
            ...
            data = {'error': True, 'data': 'Upload Valid Files Ex: docx, pdf, doc, rtf'}
        return HttpResponse(json.dumps(data))
    message = 'Sorry, Page not available, Url may be mispelled or Page not available anymore'
    return render(request, 'recruiter/recruiter_404.html', {'message': message})
...
def resume_pool(request):
        ...
        if validate_resume_applicant.is_valid():
            for resume in request.POST.getlist('applicants'):
                applied_job = AppliedJobs.objects.filter(
                    resume_applicant__id=resume, job_post__id=request.POST.get('job_post')).first()
                if not applied_job:
                    AppliedJobs.objects.create(status=request.POST.get('status'), resume_applicant_id=resume, job_post_id=request.POST.get('job_post'),
                                               ip_address=request.META['REMOTE_ADDR'], user_agent=request.META['HTTP_USER_AGENT'])
                else:
            ...
            data = {
                'error': True, 'response': validate_resume_applicant.errors}
        return HttpResponse(json.dumps(data))
    ...
    if request.user.agency_admin or request.user.has_perm('jobposts_edit'):
        ...
        agency_jobposts = JobPost.objects.filter(Q(agency_recruiters__in=[request.user]) | Q(user=request.user)).distinct()
    return render(request, 'recruiter/company/resume_pool.html', {'agency_resumes': agency_resumes,
                                                                  'recruiters': recruiters,
                                                                  'years': YEARS,
                                                                  'skills': skills,
                                                                  'aft_page': aft_page,
...
def resume_view(request, resume_id):
    ...
    if request.user.is_agency_admin:
        ...
        agency_jobposts = JobPost.objects.filter(Q(agency_recruiters__in=[request.user]) | Q(user=request.user)).distinct()
    agency_jobposts = agency_jobposts.filter(skills__in=agency_resume.skill.all().values_list(
        'id', flat=True)).exclude(id__in=applicants.values_list('job_post', flat=True))
    if request.POST.get('mode') == 'add_job':
        validate_resume_applicant = ApplicantResumeForm(request.POST)
        if validate_resume_applicant.is_valid():
            if not AppliedJobs.objects.filter(resume_applicant=agency_resume, job_post_id=request.POST.get('job_post')):
                AppliedJobs.objects.create(status=request.POST.get('status'), resume_applicant=agency_resume, job_post_id=request.POST.get('job_post'),
                                           ip_address=request.META['REMOTE_ADDR'], user_agent=request.META['HTTP_USER_AGENT'])
            ...
            data = {'error': True, 'response': validate_resume_applicant.errors}
            return HttpResponse(json.dumps(data))
    profile = render_to_string("recruiter/company/resume_view.html", {'agency_resume': agency_resume,
                                                                      'agency_jobposts': agency_jobposts,
                                                                      'status': POST,
...
def resume_edit(request, resume_id):
        ...
        if validate_resume_upload.is_valid():
            if 'resume' in request.FILES:
                ...
                if str(ftype) in sup_formates:
                    ...
                    data = {
                        'error': True, 'data': 'enter docx or pdf file only'}
                    return HttpResponse(json.dumps(data))
            ...
            if request.POST.getlist('job_post'):
                AppliedJobs.objects.filter(
                    resume_applicant=agency_resume).delete()
                for job in request.POST.getlist('job_post'):
                    AppliedJobs.objects.create(status=request.POST.get('status', 'pending'), resume_applicant=agency_resume, job_post_id=job,
                                               ip_address=request.META['REMOTE_ADDR'], user_agent=request.META['HTTP_USER_AGENT'])
    ...
    if request.user.agency_admin or request.user.has_perm('jobposts_resume_profiles'):
        ...
        agency_resume = AgencyResume.objects.filter(
            id=resume_id, uploaded_by=request.user).first()
    if agency_resume:
        template_name = 'recruiter/company/resume_upload.html'
        data = {'skills': skills, 'resume': agency_resume, 'agency_jobposts': agency_jobposts, 'status': POST}
    else:
...
def download_applicants(request, jobpost_id, status):
    ...
    for user in pending_applicants:
        ...
        writer.writerow({'email': user.user.email, 'first_name': user.user.first_name,
                         'last_name':user.user.last_name, 'location': user.user.current_city,
